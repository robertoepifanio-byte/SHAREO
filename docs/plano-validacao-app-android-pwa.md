...
| **TODO — Hero: imagem dos itens ausente** | 🔧 **implementado, PENDE confirmação em device** — `apps/mobile/assets/hero-items.webp` copiado de `public/logos/`. Adicionado à tela home entre o H1 e o subtítulo "Tudo perto de você." com dimensões 230×147px (proporção original 1100/702). Transcrito de `app/page.tsx` linha ~159. |
| **TODO — Hero: placeholder da busca não alterna** | 🔧 **implementado, PENDE confirmação em device** — `setInterval` de 3s entre os dois textos ("O que você precisa alugar?" / "O que você tem para alugar?"), limpo ao focar o campo (`onFocus`). `useEffect`+`useRef` adicionados ao topo do componente sem condicionais. Transcrito de `components/home/HeroSearch.tsx`. |
...
| **TODO — Lista VIP / captação de fundadores (`ListaVIP.tsx`)** | 🔧 **implementado, PENDE confirmação em device** — `apps/mobile/components/home/ListaVIP.tsx` e `FounderCaptureForm.tsx` criados. 1ª ocorrência (seção completa): badge "Pré-lançamento · Primeiros no Brasil", título, 4 cards de benefícios, formulário com estados colapsado/expandido/loading/sucesso/erro-rede/erro-duplicado, contador dinâmico via `GET /api/founders/stats`. Submit via `POST /api/founders/leads` (rota pública). 2ª ocorrência (colapsado, perto do rodapé): `<FounderCaptureForm />` em container navy antes do `AppFooter`. Transcritos de `components/home/ListaVIP.tsx` e `components/home/FounderCaptureForm.tsx`. |
| **TODO — Rodapé global (`AppFooter.tsx`)** | 🔧 **implementado, PENDE confirmação em device** — `apps/mobile/components/layout/AppFooter.tsx` criado. Logo (`shareo-logo.png`) + tagline "Por que comprar se você pode alugar?" + "Desenvolvido por Pratika IA", 3 colunas de links (Plataforma/Conta/Suporte, 14 links ao todo), barra inferior com copyright + `v1.0.0` + 3 selos de confiança (Pagamento seguro/Usuários verificados/Economia circular). Links nativos usam `router.push()`; demais abrem via `Linking.openURL()`. Transcrito de `components/layout/AppFooter.tsx`. |
...
| **TODO — Botão "Filtros" (bottom sheet)** | 🔧 **implementado, PENDE confirmação em device** (PR feat/mobile-explorar-todos, 2026-07-03): `FilterTriggerButton` + `FilterBottomSheet` adicionados — 4 seções verbatim do site (Categoria, Preço máx./dia, Distância, Avaliação mínima). Categoria e Preço máx. enviados à API; distância via expo-location (haversine client-side, ARQ-ALTO-09); avaliação mínima UI presente, filtro client-side (avgRating não vem de /api/items ainda). |
| **TODO — Ordenação ("Mais próximos" ▾)** | 🔧 **implementado, PENDE confirmação em device** (PR feat/mobile-explorar-todos, 2026-07-03): dropdown com as 5 opções verbatim de `_SortSelect.tsx`; `sort` adicionado ao `ListItemsQuerySchema` e ao `GET /api/items` (espelha `getOrderBy()` do site). Parâmetro `recent` é default (createdAt desc) e não enviado à API. |
| **TODO — "Ver no mapa"** | 🔧 **implementado, PENDE confirmação em device** (PR feat/mobile-explorar-todos, 2026-07-03): botão "Ver no mapa" com ícone polygon verbatim de `_MapToggle.tsx` adicionado antes do grid. Ação: `Linking.openURL(API_URL + "/itens?view=map")` — mesmo padrão do `MobileMenu.tsx` para features não portadas nativamente. Mapa nativo (Mapbox) é feature adiada; documentado explicitamente no código. |
...
- ~~**ContractBanner**, **ReturnCountdown**, **ReturnChecklist**, **ReturnConditionForm**,
  **CheckInOut** (fotos), **ReviewForm** e **bug do botão cancelar**~~ — 🔧 todos implementados
  em `feat/mobile-reservas-pos-reserva` (PR #187). PENDE confirmação em device. Auth das rotas
  contract/photos/reviews destravada no sweep (PR #188).
