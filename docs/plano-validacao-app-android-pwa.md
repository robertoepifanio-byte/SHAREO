...
| **TODO — Lista VIP / captação de fundadores (`ListaVIP.tsx`)** | ❌ **achado em teste ao vivo 2026-07-03 (revisão da decisão anterior)**: seção inteira do site ausente no mobile — badge "Pré-lançamento · Primeiros no Brasil", título "O Shareo está chegando. Entre na lista.", 4 cards (Condições especiais / Verificação de perfil gratuita / Acesso antecipado / Primeiro a descobrir), formulário `FounderCaptureForm` (captura lead, tem estado colapsado "Quero ser avisado no lançamento" + expandido), contador social dinâmico (`founderLead.count`). O mesmo `FounderCaptureForm` (colapsado) reaparece de novo mais abaixo na página, perto do rodapé — mesmo componente, 2ª ocorrência. Tinha sido decidido pular na 1ª rodada do redesign do Home (ver [[project-mobile-redesign-transcricao]]); usuário reconsiderou testando ao vivo e agora quer no TODO. Escopo maior — formulário + dado real do banco, não é só conteúdo estático |
| **TODO — Rodapé global (`AppFooter.tsx`)** | ❌ **achado em teste ao vivo 2026-07-03**: rodapé inteiro ausente no mobile — logo+tagline "Por que comprar se você pode alugar?"+"Desenvolvido por Pratika IA", 3 colunas de links (Plataforma/Conta/Suporte, 14 links ao todo), barra inferior com copyright+versão+3 selos de confiança (Pagamento seguro/Usuários verificados/Economia circular). Native apps normalmente não têm "rodapé" (a navegação já fica na BottomNav) — decisão de implementar ou não fica pro time, por ora só registrado |
