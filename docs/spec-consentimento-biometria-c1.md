...
2. **Coletar o consentimento** num passo dedicado da UI, com checkbox separado e texto integral (vide `docs/consentimento-biometria-texto-c1.md`).
...
|---|---|---|
| `app/perfil/_IdVerification.tsx` (linhas 130-211 do modal) | Abre modal com botoes "Tirar foto" / "Galeria" diretamente, sem checkbox especifico para a selfie. Texto generico "Seus dados sao protegidos pela LGPD" (linhas 185-188) **nao serve** como consentimento destacado. | Antes do bloco da selfie (a partir da linha 163 — "Selfie segurando o documento"), inserir um **passo dedicado** (sub-modal ou step) com o texto integral de `docs/consentimento-biometria-texto-c1.md` e checkbox separado. Botao "Tirar selfie" / "Galeria" **so habilita** apos o checkbox. Estado novo: `biometricConsentAccepted: boolean`. |
...
Cross-ref: a auditoria s40 (`docs/auditoria-conformidade-tecnica-s40.md` secao 1) recomenda Log Drain para retencao MCI art. 15. Esses eventos entram no mesmo pipeline.
...
- `docs/redline-ripd-biometria-c1.md` — adendo ao RIPD reclassificando a selfie como dado biometrico, atualizando F-09 e medidas de seguranca.
- `docs/consentimento-biometria-texto-c1.md` — texto integral do consentimento que sera exibido ao titular (a aprovar pelo DPO).
- `docs/spec-consentimento-biometria-c1.md` — este documento.
...
9. RIPD assinado com Secao C.3, B, E, F e H atualizadas conforme `docs/redline-ripd-biometria-c1.md`.
