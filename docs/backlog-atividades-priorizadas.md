...
|---|---|---|
| **ATOR-01** ⚠️dinheiro | `resolve_cancelled` (disputa a favor do locatário) **não grava `refundAmount`**. O caminho equivalente fora da disputa (`cancel`) grava, via `calcRefund`. Hoje a disputa termina em CANCELLED e nada registra quanto devolver — e o estorno é executado à mão no painel da Stripe. | O **valor** é decisão de política. `calcRefund` aplica a escada por proximidade da retirada (100% / 70% / 50%), que faz sentido em desistência e **não** em disputa: quem recebeu item quebrado não deveria levar 50% por ter reclamado tarde. Escolher entre "integral" e "escada" é do fundador. |
