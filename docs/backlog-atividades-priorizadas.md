...
- **23/09/2026 — preparado pelo técnico, nada fechado:**
  ...
  - ✅ **Check "Vercel" da campanha vermelho em todo PR desde 31/08 — causa confirmada e corrigida em 24/09:** o build de preview falhava sem `NEXT_PUBLIC_SHAREO_API_URL` (guard de `lib/config.ts`) porque as três `NEXT_PUBLIC_*` do projeto `shareo-campanha` só existiam no escopo Production. O fundador marcou Preview nas três; o redeploy do preview do #498 passou (Ready, 53 s) contra o original em Error (27 s). Nota: o Preview usa a MESMA URL de produção (não uma linha só de Preview apontando ao staging); risco de lead de teste cair na base de produção, pequeno se a Deployment Protection dos previews estiver ligada (não conferido).
