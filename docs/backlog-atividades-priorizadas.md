...
| **🔴 N1 — CTA de reserva mobile aponta p/ rota inexistente (404)** | 🔴 **decisão do fundador** | Revisão de navegabilidade s41 (CONFIRMADO): `app/itens/[id]/_StickyBookingCTA.tsx:36` manda usuário logado p/ `/reservas/nova?itemId=…`, que **não existe** → 404 no botão fixo "Reservar agora" em mobile. Fix sugerido: **rolar até `#price-calc`** (o componente já tem lógica de esconder quando a calculadora está visível) em vez de navegar. Bug pré-existente; conversão em mobile. |
| **a11y do modal de KYC** | 🟠 **decisão do fundador** | Revisões UI/UX+código s41: tap targets <44px (`h-9`/`py-2.5` nos botões do modal e no botão Verificar/Reenviar), falta `role="dialog"`/`aria-modal`/trap de foco/fechamento por Escape. Pré-existente. **Fechar antes de ligar a flag `biometricConsentRequired`** (fluxo de consentimento jurídico). |
| **Dark mode da Central de Ajuda** | 🟠 **decisão do fundador** | Revisão UI/UX s41: 5 das 7 seções de FAQ + os callouts (Dica/Exemplo/Atenção) usam `bg-*-50 border-*-200` Tailwind hardcoded sem override `dark:` → quebram no modo escuro. Também `text-amber-600` (status PENDING) fora do token `text-booking-pending`. Só cor; dark mode já gated pós-D4. |
| **Prep pró-flag de biometria (Etapa 2)** | 🔵 **pós-D4** | Antes de ligar `biometricConsentRequired`: (1) botão "Revogar consentimento biométrico" na UI (`_IdVerification`, status VERIFIED) — endpoint DELETE já existe; (2) incluir `idSelfieConsentAt/Version/TextHash` no export art.20 (`users/me/export`), **exceto** `...Ip`; (3) log `kyc.selfie.view` no admin; (4) adendo à Política citando art.11 II "a". Ver `docs/spec-consentimento-biometria-c1.md` (DoD). |
