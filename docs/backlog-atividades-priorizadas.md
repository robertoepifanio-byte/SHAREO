...
- **23/09/2026 — preparado pelo técnico, nada fechado:**
  - ✅ **GTM declarado** nas duas Políticas (site, campanha e app) e teste que reprova se o GTM ficar ligado sem declaração. **Mesclado em 24/09 (#498) e no ar em shareo.com.br** (conferido: `/politicas` e `/privacidade` citam o Google Tag Manager). O app mobile só recebe o texto no próximo build. **Em 24/09/2026 o Raimundo (Encarregado) confirmou que a declaração basta**, então não é preciso desligar o `GTM_LIBERADO` (F-13 do RIPD quanto à declaração). Confirmação verbal, sem registro escrito da advogada: vale pedir um e-mail curto para o dossiê do D4. **Não cobre** a promessa "não lemos o que você digita" (depende do console do GTM; trava de código no #499) **nem o art. 33** (abaixo).
  ...
  - 🔴 **Art. 33:** releitura em `docs/juridico/art33-fornecedores-recheck-2026-09-23.md`. Nenhum fornecedor mudou; o **Google (GTM)** entra como sexto sem CPC. Depende da advogada. A confirmação de 24/09 sobre a declaração do GTM (item acima) trata do texto da Política, não desta transferência internacional.
  - ✅ **Encarregado unificado (site e campanha):** a Política §2.7 publica **Raimundo Gomes da Silva**, como no RIPD, por uma constante única (`DPO_NOME`) ligada ao RIPD por teste. Mesclado em 24/09 (#498) e conferido no ar: `/politicas` de shareo.com.br traz o nome, sem o de Roberto. O app mobile só recebe no próximo build.
  ...
  - ✅ **Check "Vercel" da campanha vermelho em todo PR desde 31/08 — causa confirmada e corrigida em 24/09:** o build de preview falhava sem `NEXT_PUBLIC_SHAREO_API_URL` (guard de `lib/config.ts`) porque as três `NEXT_PUBLIC_*` do projeto `shareo-campanha` só existiam no escopo Production. O fundador marcou Preview nas três; o redeploy do preview do #498 passou (Ready, 53 s) contra o original em Error (27 s). Nota: o Preview usa a MESMA URL de produção (não uma linha só de Preview apontando ao staging); risco de lead de teste cair na base de produção. Conferido em 24/09: o preview responde 302 para o login SSO da Vercel, então só quem está logado no time o abre. Depois do push, o check verde apareceu no GitHub.
...
| ~~**PSP-02**~~ | ~~Código do Mercado Pago virou peso morto.~~ | 🟢 **RESOLVIDO em 2026-08-24** — o fundador mandou arrancar. SDK, rotas, UI (web e app), flag, testes e script removidos; a decisão nº 6 do ADR-028 fica revogada. **Sobram os campos de banco** — ver PSP-04. |
| **PSP-03** | **O app ficou sem caminho de pagamento próprio.** O botão "Pagar reserva" chamava `/api/payments/mp/checkout`; virou atalho para pagar no site. | ⚖️ **As duas pernas NÃO custam o mesmo** — estimar junto leva a erro. **Barata:** `/api/payments/checkout` autentica por `auth()` (cookie) e o app precisa de `resolveUserId`; o padrão GET(web)/POST(app) já está pronto em `/api/payments/stripe/connect` e já é consumido pelo app. ~5 linhas. **Cara:** o RETORNO, que **nunca existiu** — a rota do MP mandava `back_urls` para `shareo://pagamento/sucesso`, mas não há rota `pagamento` em `apps/mobile/app/` nem listener de deep-link no app inteiro; e `success_url` da Stripe é http(s), não aceita scheme customizado. Precisa de página-ponte no site ou `openAuthSessionAsync`, mais teste em device. **Nada regrediu** — com a flag OFF o botão já só dava "indisponível". |
| ~~**PSP-04**~~ 💥 | ~~Campos de banco do MP ainda existem.~~ | 🟢 **RESOLVIDO em 2026-08-24** — migração `20260824190000_remove_mercado_pago`: 9 colunas, a tabela `mercado_pago_event_queue` e a linha `mercadoPagoEnabled` de `platform_configs`. Destrutiva, autorizada com escopo explícito após conferir o que havia em staging (dados de sandbox). **Ensaiada contra o banco real dentro de transação revertida** antes do merge — foi o ensaio que pegou `platform_config` no singular, que abortaria a migração e travaria todos os deploys seguintes. |
| ~~**LEGAL-01**~~ ⚠️legal | ~~**Endereço da sede ausente** no bloco de identificação.~~ | 🟢 **RESOLVIDO em 2026-08-24** — o fundador enviou o **Comprovante de Situação Cadastral**: `Rua Pais Leme, 215, conj. 1713 — Pinheiros, São Paulo/SP, CEP 05424-150`. Endereço **virtual** (escritório), que é o que consta como sede na Receita e é exatamente o que o Decreto 7.962/2013, art. 2º, I manda publicar. Teste com o valor real trava a regressão para `null`. Com isso o EVAL-P0-01 está **completo**. |
...
|---|---|---|---|---|
| ~~**EVAL-P0-01**~~ ⚠️legal | ~~`/termos` e `/privacidade` **não identificam a ShareO como PJ**.~~ | `lib/legal-config.ts` (`LEGAL_ENTITY`), `components/legal/IdentificacaoPrestador.tsx` | 🟢 **RESOLVIDO em 2026-08-24** — bloco de identificação (razão social + CNPJ + contato) em `/termos`, `/privacidade` e `/politicas`, mais as duas telas do app. Fonte única; teste tranca as 5 telas. | ✅ **Completo desde 24/08** — razão social (a da Receita, não a de rascunho), CNPJ e endereço da sede publicados. Ver LEGAL-01. |
| **EVAL-P0-02** | Campos de localização (Cidade/Estado/Bairro/Endereço) no **modo create** parecem editáveis mas ignoram o input silenciosamente (`onChange` faz `if (mode !== "create") …`, sem `disabled`/`readOnly`). | `components/items/ItemForm.tsx` | ✅ **RESOLVIDO — já estava.** Os quatro campos têm `disabled={loading || mode === "create"}`; a descrição acima envelheceu. Conferido em 04/09/2026 | Proprietário que não lê o banner tenta digitar e trava. Fix: `readOnly`/`disabled` quando `mode==="create"`. |
| **EVAL-P0-03** | **Chips de categoria no `/itens` mobile ~30-34px** (`py-1.5 text-xs`), abaixo dos 44px do DS. | `app/itens/page.tsx:370,387` | ✅ **RESOLVIDO em 04/09/2026** — `min-h-11` nos dois chips (`app/itens/page.tsx`) | Filtrar por categoria fica difícil no mobile (tap target). |
| **EVAL-P0-04** | **Política de cancelamento contraditória entre telas:** página do item mostra faixas hardcoded; `/ajuda` diz outra coisa. | `app/itens/[id]/page.tsx:625`, `app/reservas/sucesso/page.tsx:171` | ✅ **RESOLVIDO em 04/09/2026.** Os 2 pontos prometiam a política **abolida em 25/08** — `/reservas/sucesso` dizia "gratuito até 24h antes" (a antecedência deixou de importar) e a página do item dizia "reembolso integral", omitindo a taxa da Stripe que o locatário absorve. Texto alinhado ao `calcRefund` e à `/ajuda`, que já estavam corretos. Teste em `cancellationPolicy.test.ts` reprova a volta do texto antigo (provado por mutação) | A divergência agora é entre config e código, não entre dois textos. Fonte única da verdade = `lib/platform-config.ts`. Achado da **jornada viva**. |
...
| **EVAL-P0-06** | Threshold de cobertura global em **1% de linhas** — sem enforcement real; meta H1 de 70% sem guardrail no CI. | `jest.config.ts` | ✅ **RESOLVIDO em 04/09/2026** — catraca em `lines: 33 / functions: 41 / branches: 72` (medido: 34,14% / ~43% / 73,54%) | Regressão de cobertura passa despercebida. |
...
| ~~**SEC-MAJ-06** ⚠️LGPD~~ ✅ **RESOLVIDO** (verificado 2026-08-19, reconfirmado 2026-08-23) | ~~DELETE de conta anonimiza nome/e-mail/CPF mas **não** `borrowerNote/ownerNote/Review.comment/Message.content/OwnerPaymentAccount(PIX)/IDVerification.idDocumentUrl+selfie`~~ — o DELETE atual faz scrub atômico de TODOS esses campos: `Review.comment→null`, `Message.content→"[mensagem removida]"+deletedAt`, `borrowerNote/ownerNote→null`, `pixKey→"REMOVIDO"`, `idDocumentUrl/idSelfieUrl→null`, arquivos `id-docs` removidos via `after()`. ⚠️ **Correção (24/09/2026): esta última parte NÃO era verdadeira** — a rotina listava `id-docs` por `<userId>`, mas o upload grava em `id-verification/<userId>/…`, então nada era removido e o documento e a selfie ficavam no bucket; o "verificado" de 19 e 23/08 foi por leitura, sem prova contra o Storage. Conserto **implementado, aguardando verificação em staging** (`lib/supabase/purge-user-storage.ts`); acompanhar no item 6 de `docs/checklist-go-live-2026-10-01.md`. | `app/api/users/me/route.ts` | ✅ Sem violação de LGPD art.18 **nos campos do banco**; nos arquivos do Storage, ver a correção ao lado. Só ficam retidos valores/datas de transações concluídas (CTN art.173, 5 anos, com aviso ao titular — permitido pelo art.18 §3º). Baixa dada conforme decisão do fundador (avaliação multi-perfil 2026-08). |
...
- **Bloqueador D4:** ~~SEC-MAJ-06~~ (✅ resolvido, verificado 2026-08-19; ⚠️ arquivos do Storage: ver correção de 24/09 na tabela MAJOR) + LGPD-01/03 são exatamente o que o jurídico vai cobrar — antecipar no dossiê da consulta.
