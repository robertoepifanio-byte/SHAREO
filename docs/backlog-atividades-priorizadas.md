...
| Item | Status | Ação |
|---|---|---|
| **📱 Distribuir build preview do app aos testers** | 🟡 **bloqueado — cota EAS** | Preview novo do main atual não pôde ser gerado: **cota de builds Android do plano Free do EAS esgotada** (msg "used its Android builds from the Free plan this month, which will reset in 22 days" → reset ~**31/07/2026**). O build passou do prebuild (fix do `runtimeVersion` em #232) e só parou na cota. **Único APK pronto = `81a980f6`, mas do commit `40fd378` (antes dos chips corrigidos/Moda removida/iOS) → NÃO distribuir (mostraria versão bugada).** Ação: **aguardar reset da cota (~31/07)** OU **upgrade EAS p/ Pro** (pago) para buildar já. Decisão do fundador. |
