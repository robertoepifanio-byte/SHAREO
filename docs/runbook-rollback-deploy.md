...
- **Marque o retorno com um nome que NÃO dispara deploy**: tag `prod-ok-AAAA-MM-DD-HHMM` (hora UTC; nunca `web-v*`, que publicaria na hora; prefixo `v*` sem `web-` também não dispara). Crie depois de cada deploy de produção que passou nas sondas: `TAG=prod-ok-$(date -u +%Y-%m-%d-%H%M); git tag $TAG <sha-do-deploy-verde>; git push origin $TAG`. A hora no nome é de propósito: o mesmo dia pode ter mais de um deploy verde (o D-2 tem deploy e ensaio de rollback; o D0 pode ter dois), e `git tag` recusa nome repetido. **Nunca force nem mova uma tag `prod-ok-*`:** isso apaga o ponto de retorno. Existe uma, criada em 25/09 a pedido do Roberto: **`prod-ok-2026-09-24-2334`**, no `a3c3a040` (#508, o deploy de produção de 24/09 às 23:34 UTC; sondas P1 a P6 verdes em 25/09; o push não disparou workflow). O `/api/health` não traz commit, então "a produção roda o `a3c3a040`" vem do histórico do workflow (nenhum deploy de produção depois dele) e do `upstashNs` de produção no health, não de prova direta: confirme na página Domains da Vercel antes de contar com ela. Ainda não foi usada num rollback. Tag em push não aciona a CI (`main.yml` escuta só branches).
...
```bash
gh workflow run deploy.yml --ref prod-ok-2026-09-24-2334    # tag ou branch do ponto de retorno (o `--ref` do gh aceita nome de branch ou tag)
