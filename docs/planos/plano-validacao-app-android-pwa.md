...
| Detalhe — **pull-to-refresh** (achado 2026-07-09) | 🔧 **achado + corrigido (PR #221):** a tela de detalhe **não tinha pull-to-refresh** (só as listas tinham); o fundador precisava voltar+reabrir pra ver mudanças de status. Adicionado `RefreshControl` ligado a `isRefetching`/`refetch`. Pende re-teste (dev-client preso em cache). |
...
| Detalhe — **CheckInOut** (fotos de check-in/check-out) | ✅➕🔧 **CONFIRMADO EM DEVICE 2026-07-09** — o botão abre o `expo-image-picker` **sem crashar** (módulo nativo OK). 🔧 **achado + corrigido (PR #221):** o botão dizia "Tirar foto ou escolher da galeria" mas só abria a **galeria** (`launchImageLibraryAsync`) — nunca a câmera. Fix: helper `pickImageAsset()` oferece **câmera OU galeria**. Fix pende re-teste (dev-client preso em cache). |
...
ausente) seguindo `docs/planos/meta-app-android-retranscricao-s41.md`, em 6 iterações
