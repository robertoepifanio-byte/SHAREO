...
|---|---|---|
| **Conta do usuário** | Clique do titular em "Excluir conta" (`DELETE /api/users/me`) | **Imediato**, em uma transação: anonimiza nome, e-mail, telefone, bio, avatar, localização, documentos (hash e cifra), senha, comentários e mensagens; em segundo plano, logo após a resposta, apaga do Storage o documento e a selfie (`id-docs`), os uploads do titular em `uploads/<id>` (avatar, fotos de avaliação e de relato de problema), **todas as fotos dos anúncios do titular** (`item-images/<itemId>/`) e **as fotos de check-in/out carregadas pelo titular** (`booking-photos/bookings/<id>/…`) — fotos da outra parte da locação são preservadas. Decisão do fundador 25/09/2026: todas essas fotos são apagadas na exclusão. Sessões web e tokens mobile são invalidados imediatamente. Falha de Storage não impede a exclusão (a conta é anonimizada de qualquer forma), mas gera alerta no Sentry para intervenção manual. Recusada se houver locação em andamento. Registros financeiros dos últimos 5 anos ficam **anonimizados**, com o titular informado na hora |
