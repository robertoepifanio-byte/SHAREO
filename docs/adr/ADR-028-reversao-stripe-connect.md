...
- [ ] **Split no checkout (transfers/destination) ainda não implementado — e precisa ser desenhado em cima da Accounts v2, não v1.** O item 2 desta decisão ("destination charge: `application_fee_amount` + `transfer_data.destination`") descreve o mecanismo clássico da API v1. Como o onboarding já foi implementado em v2 (ver "Notas de implementação" — Accounts v1 foi recusada pela Stripe para contas novas), o checkout/split precisa confirmar o equivalente v2 antes de implementar — não assumir que `transfer_data.destination` funciona sem adaptação numa `PaymentIntent` visando uma connected account v2.
...
- **Plano técnico restante** (fora do escopo desta mudança de schema):
  ...
  2. Checkout com Pix + boleto + cartão à vista e split — **precisa ser desenhado pra Accounts v2** (ver "Riscos/Pendências"), não é mais só `transfer_data`/`application_fee_amount` de v1 sem adaptação.
