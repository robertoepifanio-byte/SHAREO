...
**Atualizado em**: 2026-06-15 (sessão s18 — **staging DESBLOQUEADO e no ar** via merge dos PRs #17+#18+#19; reds do CI diagnosticados; **e-mail do app AO VIVO** — sandbox Resend resolvido + domínio verificado + `EMAIL_FROM` corrigido; **staging migrado para `staging.shareo.com.br`**; **caixas humanas: Zoho Mail Free escolhido pelos fundadores** — implantação pendente)
**Ambiente staging**: https://staging.shareo.com.br (canônico desde 15/06; `shareo-rouge.vercel.app` segue como alias) — ✅ **DESBLOQUEADO (s18): no ar com o código atual** (`/seguranca`, `/.well-known/security.txt`, `/api/items` = 200; ondas 3/4/s15/s16 finalmente subiram). O job *Deploy Staging* do GitHub Actions deployou via token — **não precisou** da ação manual no dashboard Vercel (o blocker real era o build quebrado). Reds remanescentes do CI são de **carga/qualidade, não bug** — ver Resumo. Diagnóstico: `docs/diagnostico-deploy-staging.md`.
**Último commit**: `1753e29` (na `main`, PRs #17/#18/#19 — **deployado no staging** ✅)
...
**✅ s18 (2026-06-15) — E-mail do app AO VIVO + staging no domínio oficial. 🟡 Provedor das caixas humanas em deliberação dos fundadores.** Três frentes nesta sessão, todas validadas por logs de runtime do staging:
- **(1) Sandbox do Resend RESOLVIDO — e-mail do app entrega de verdade.** Diagnóstico do "esqueci a senha" mostrou que o backend sempre funcionou (token gerado), mas **nenhum e-mail real chegava**: o Resend estava em sandbox (domínio não verificado) **e** o `EMAIL_FROM` no Vercel era `onboarding@resend.dev` (remetente sandbox, só entrega ao dono da conta, em spam). Fundador **verificou `shareo.com.br` no Resend** (DKIM `resend._domainkey` + SPF `send`, via DNS GoDaddy). Fix aplicado via CLI: `EMAIL_FROM` → **`noreply@shareo.com.br`** (Production) + `vercel redeploy` (35s, sem rebuild de código). **Validado:** POST forgot-password p/ endereço **não-dono** ANTES = `error … You can only send testing emails to your own email address`; DEPOIS = `warning [forgot-password] email sent`. Agora entrega p/ **qualquer destinatário, na inbox**, assinado DKIM/SPF. Vale p/ reset, verificação de cadastro e e-mails de reserva. Follow-up menor: `EMAIL_FROM` ficou só em Production (bug do `vercel env add` p/ Preview na CLI v54).
- **(2) Staging migrado para o domínio oficial `https://staging.shareo.com.br`.** Domínio atachado ao projeto Vercel; **registro `A staging → 76.76.21.21`** adicionado na GoDaddy (não delegação de NS — **nunca** trocar NS p/ Vercel, quebraria o e-mail); SSL automático; HTTP 200 + HSTS. `AUTH_URL`+`NEXTAUTH_URL` (Production) → `https://staging.shareo.com.br` + redeploy → **links de e-mail e redirects de login agora usam o domínio oficial** (validado: `email sent` servido por `staging.shareo.com.br`). `shareo-rouge.vercel.app` segue como **alias ativo**. **Apex `shareo.com.br` + `www` RESERVADOS** para a prod real (pós-D4) — hoje o apex ainda aponta p/ o "WebsiteBuilder Site" placeholder da GoDaddy. `CLAUDE.md:8` atualizado. 1 só projeto Vercel (`shareo`): env "Production" = staging; não há prod separada.
- **(3) ✅ Caixas humanas — DECIDIDO: Zoho Mail Free (R$0 no 1º ano), migrar p/ pago quando faturar. Implantação pendente.** Varredura do código fixou as caixas que o produto **exige** (todas `@shareo.com.br`): **`suporte@`** (atendimento), **`privacidade@`** (LGPD — exclusão/opt-out), **`seguranca@`** (`security.txt`). `noreply@` = só remetente Resend (alias→suporte@); `admin@`/`financeiro@`/`operacional@` = logins do painel (não canais). **Trilha da decisão:** no **GoDaddy Pro Light (R$6,49)** o alias **só recebe** — enviar COMO o endereço exige **Premium+** (Pro Plus R$10,99 não tem send-as; **Premium R$18,99 tem**; 1 Premium < 3 Pro Light R$19,47). A recomendação inicial foi **GoDaddy Premium**, mas os fundadores optaram por **começar no Zoho Mail Free**: o Free dá **até 5 usuários** num domínio próprio → criar os 3 endereços como **usuários REAIS separados** (cada um envia/recebe como si, **some o problema de send-as**), R$0. **Única limitação aceita:** Free **sem IMAP/POP/SMTP** (só webmail `mail.zoho.com` + app mobile; cliente desktop só no pago/migração futura). **Implantação (pendente):** fundadores criam conta Zoho + add domínio + 3 usuários + ativam DKIM (precisa login Zoho, agente não faz); agente entrega a **folha de DNS** p/ GoDaddy (raiz `@`: **MX** `mx.zoho.com`/`mx2`/`mx3`, **SPF** `v=spf1 include:zoho.com ~all`, **TXT verificação** + **DKIM `zmail._domainkey`** do wizard; **DMARC** já existe `p=quarantine`). **NÃO TOCAR:** `resend._domainkey`/`send` MX/`send` TXT (app Resend — SPF raiz do Zoho não conflita, Resend usa DKIM próprio + Return-Path em `send.`), `A staging→76.76.21.21` (Vercel), `A @→WebsiteBuilder` (web). App segue enviando `noreply@` pelo Resend (inalterado).

**✅ s18 (2026-06-15) — Staging DESBLOQUEADO e no ar + diagnóstico completo dos reds do CI.** Os 3 fixes do pipeline foram para PR (delegados a subagentes devops + arquiteto em worktree; o de teste feito pelo orquestrador) e **mergeados na `main`** (squash, `60027fb`/`7577e57`/`1753e29`): **[#17](https://github.com/robertoepifanio-byte/SHAREO/pull/17)** — lint de `app/seguranca/page.tsx:29` (`"https://"` → `{'"https://"'}`), destrava o `next build`; **[#18](https://github.com/robertoepifanio-byte/SHAREO/pull/18)** — separa o `ALTER TYPE "ItemStatus" ADD VALUE 'DELETED'` numa **nova migration de timestamp anterior** (`20260603050000`), **sem editar a já aplicada** (checksum preservado em local/staging); **ADR-023** documenta a escolha; ordem verificada (`CREATE TYPE "ItemStatus"` está em `20260603000000`, antes da nova); **[#19](https://github.com/robertoepifanio-byte/SHAREO/pull/19)** — **3º blocker descoberto na revisão**: o job `Tests` da CI morria no `prisma migrate deploy` (`55P04`) **antes do jest rodar**, mascarando **2 testes** de `__tests__/integration/api/bookings/patch.test.ts` cujo mock de prisma não acompanhou a blindagem TOCTOU da s14 (`confirm`→`$transaction`, `mark_active`→`updateMany`+`findUniqueOrThrow`); mock corrigido, `npx jest --ci` **24/24 suites, 450 passed**. **Com os 3 na `main`, o job *Deploy Staging* do GitHub Actions deployou via token e o staging subiu** (`/seguranca`, `security.txt`, `/api/items` = 200). O *Ignored Build Step* do dashboard Vercel era **secundário** — o blocker real era o build quebrado, que também travava o deploy via Actions. **Reds remanescentes do CI da `main` DIAGNOSTICADOS** (via `vercel logs` + cadastro de teste no staging, autorizado): **(1) `register`/`/api/items` 500 da E2E = CARGA/INFRA, não bug** — curl direto sequencial dá 201/200/409; `register` e `ENCRYPTION_KEY` OK; **drift descartado** (`prisma migrate status` no staging = "Database schema is up to date"); a suíte `e2e/*-plan.spec.ts`+`e2e-suite.spec.ts` dispara rajadas que esgotam o pool do Supabase free → 500 genérico em register e items juntos (flakiness do gate sob burst). **(2) 🔗 Resend em modo SANDBOX, sem domínio verificado** → o app só envia para `roberto.epifanio@gmail.com`; **entrega de e-mail real está bloqueada** hoje (logs do register: `verify a domain at resend.com/domains`) — resolve com a implantação de e-mail abaixo. **(3)** 3 violações de **contraste WCAG AA** (`e2e-a11y-plan.spec.ts`). **(4)** Lighthouse **`errors-in-console`** em `/`, `/itens`, `/login`. **Lição reforçada:** destravar um pipeline congelado há muito **expõe** gates e bugs que estavam mascarados por falhas upstream (mesmo padrão do #19). Test user criado no staging para o diagnóstico (`cmqfotsbt0001` / `debug-reg2-*@shareo-test.com`) **mantido por decisão do fundador** (não limpar).

**📧 s18 (2026-06-15) — Decisão de provedor de e-mail + plano de implantação em 5 fases.** ⚠️ **PARCIALMENTE SUPERADA pela entrada do topo:** a Fase 2 (verificar `shareo.com.br` no Resend) **já foi feita e o e-mail do app está AO VIVO**; e a escolha "Pro Light" está **em revisão** — descobriu-se que no Pro Light o alias **só recebe** (send-as exige Premium R$18,99), então a recomendação virou **Premium 1 caixa**, agora em deliberação dos fundadores. Registro original abaixo: Fundador decidiu: **GoDaddy Pro Light** para as caixas humanas da equipe (`contato@`, `suporte@`, `seguranca@` — esta última obrigatória, já publicada no `security.txt`) + **Resend** para o transacional automatizado do app. Estado real do DNS de `shareo.com.br` (consultado): **NS = GoDaddy** (`domaincontrol.com` — tudo num painel só), **MX vazio** (sem caixas hoje), **SPF/DKIM ausentes**, **DMARC já = `p=quarantine`** (relaxado) → e-mails sem autenticação já tendem ao spam. Plano: **(1)** assinar Pro Light + criar caixas (GoDaddy auto-provisiona MX/SPF/DKIM dela); **(2)** verificar `shareo.com.br` na **raiz** no Resend (mantém From = `noreply@shareo.com.br`, zero mudança de código — Resend isola o Return-Path no subdomínio `send.` → sem conflito de SPF); **(3)** adicionar os registros DNS no painel GoDaddy (dica de cutover: DMARC `p=none` enquanto valida, depois volta a `quarantine`); **(4 — código, do orquestrador)** adicionar `Reply-To: suporte@shareo.com.br` nos envios + confirmar `EMAIL_FROM`; **(5)** validar DKIM/SPF/DMARC = pass (mail-tester). Fases 1–3 são do fundador (contas/DNS, sem acesso do agente); **Fase 4 pendente do "vai".** O `sendAppEmail()` (PR #16) já é o **ponto único de integração** com o provedor.

**⚠️ s17 (2026-06-14) — Pipeline de deploy do staging DESCOBERTO QUEBRADO + bateria E2E + bug real corrigido.** Ao rodar a **bateria E2E geral** contra staging (255✅/16❌/19skip, 290 testes, 0 bugs de app) e corrigir os 4 itens de manutenção de teste apontados, surgiram **2 descobertas**: (a) `e2e/security2.spec.ts #21` chamava `mark_active` **sem `pickupToken`** (bug no spec, mascarado por contaminação de dados) → corrigido; (b) **bug REAL de produção** em `app/api/auth/forgot-password/route.ts` — a resposta de sucesso era um **`NextResponse` singleton de módulo**; como o body de uma Response é stream consumível **uma única vez**, a 2ª+ requisição na mesma lambda quente saía **HTTP 200 com body vazio** (a maioria dos usuários de "esqueci a senha" recebia resposta sem corpo). Convertido para função `ok()`; verificado local (3 POSTs → 3× body). Os 4 itens de teste (email-verify 400, locators de logout/senha no `auth.spec`, header `x-e2e-token` no config de staging, pré-limpeza de bookings no `globalSetup`) + as 2 correções foram para **PR [#15](https://github.com/robertoepifanio-byte/SHAREO/pull/15), mergeado na `main`** (`bea8da7`+`c506bba`). **Ao tentar o deploy, descobriu-se que o pipeline de staging está quebrado** (ver cabeçalho + `docs/diagnostico-deploy-staging.md`): (1) a `main` **não builda** — `next build` falha no lint de `app/seguranca/page.tsx:29` (`react/no-unescaped-entities`) desde a onda-3, e não há `eslint.ignoreDuringBuilds`; (2) o Vercel **cancela** os deploys de git (`Ignored Build Step`). Resultado: **onda 3/4/s15/s16 + o fix do forgot-password nunca chegaram ao staging.** Blocker correlato: a CI (`Tests`) falha em `prisma migrate deploy` do zero com `55P04 unsafe use of new value "DELETED" of enum ItemStatus` — **vai travar a criação do Supabase de produção pós-D4** (separar `ADD VALUE` do uso em 2 SQLs). **Decisão do fundador: pausar e investigar o pipeline com calma — nenhum deploy forçado.** Remediação mínima registrada no diagnóstico: PR do fix de lint (1 linha) → `npx vercel --prod`.

**✅ s17 (2026-06-14) — Estratégia de provedor de e-mail + 2 limpezas da auditoria (PR #16).** Análise da troca **Resend → GoDaddy**: são categorias diferentes — Resend = **envio** (API, ideal p/ serverless); GoDaddy Pro Light = **caixa/inbox** (os "50.000" do plano são e-mails *armazenados*, não enviados). **Tendência do fundador:** manter **Resend** para o transacional do app (Free → Pro ~US$20/mês quando houver ~20–30 reservas/dia — o **limite de 100/dia** do Free quebra antes do mensal, porque cada reserva espalha ~5 e-mails e os crons disparam em lote) + **GoDaddy Pro Light** só para as **caixas humanas** (`contato@`/`suporte@`, receber/responder). Pega de implementação (1 domínio, 2 senders): SPF deve incluir os dois, DKIM de cada, DMARC alinhado, **MX → GoDaddy**, e `Reply-To` das automáticas → `suporte@`. **2 extras da auditoria de e-mail RESOLVIDOS** em PR [#16](https://github.com/robertoepifanio-byte/SHAREO/pull/16) (branch `chore/email-cleanup-dead-code`, `9ffa609`): (1) removidos `sendWelcomeEmail` + helper `welcomeHtml` (código morto; o mock do `register.test` apontava errado p/ eles → corrigido p/ `sendVerificationEmail`); (2) cron de reengajamento deixou de instanciar o SDK Resend direto → usa o novo `sendAppEmail()` de `lib/email.ts`, **1 único ponto de integração** com o provedor. Verificado: `tsc` limpo, lint sem novos erros, `register.test` 10/10. **Sem dependência externa; não deployado (pipeline pausado).** Detalhe: o repo está em double-quote apesar do `.prettierrc` (`singleQuote: true`) e o hook de pré-commit não está ativo — débito de consistência separado.
