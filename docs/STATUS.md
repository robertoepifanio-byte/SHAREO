...
**Atualizado em**: 2026-06-20 (sessão s32 — **KYB leve no cadastro de PJ MERGEADO e no ar no staging** (PR #63); s31 = modo escuro; s30 = planejamento dark mode; s29 = campanha #50 + Vercel Pro + Zoho + v1.8.0; histórico s13–s29 abaixo)
...
**Último commit**: KYB leve PJ mesclado na `main` (PR [#63](https://github.com/robertoepifanio-byte/SHAREO/pull/63) squash); deploy + E2E Regressão Contínua verdes no staging após fix do incidente de migração (ver s32).
...
**🏢 s32 — KYB leve no cadastro de PJ (verificação de CNPJ + responsável legal) MERGEADO e no ar no staging** (2026-06-20). Originou-se de uma pergunta do fundador sobre o cadastro de PJ → parecer do `product-owner-shareo` (benchmark de mercado: Mercado Livre/Amazon/OLX/Airbnb fazem KYB; ShareO só validava dígito verificador) → **ADR-024** (`arquiteto-shareo`) + parecer de `seguranca-shareo` → user story → implementação. **Entrega (PR [#63](https://github.com/robertoepifanio-byte/SHAREO/pull/63), squash):** **M1** consulta a situação cadastral do CNPJ na Receita (`lib/pjVerification.ts` — BrasilAPI + fallback Minha Receita, timeout 5s/retry, cache Upstash), rejeita inativo/baixado/suspenso; **M2** coleta nome do responsável legal + declaração auditada (art. 299 CP, com IP/data, texto em `PJ_DECLARATION_TEXT`); **M3** persiste razão social/situação/data de abertura; **M4** exige cadastro PF completo antes de virar PJ e **unifica os dois pontos de entrada** (`upgrade-pj` + `complete-registration` com `userType=PJ`, que passou a exigir CPF do responsável → toda PJ no banco tem `cpfHash!=null AND cnpjHash!=null`). **Decisão-chave do fundador: fail-open controlado** (sobre o fail-closed proposto pelo Arquiteto) — Receita indisponível grava PJ em revisão (`cnpjSituacaoVerificada=false`) sem bloquear, com fila admin **`/admin/usuarios/kyb-pendentes`** ("Verificar na Receita" / "Aprovar manualmente"); como o payout já é gated por D4, PJ em revisão não move dinheiro. **Segurança:** `encryptPII()` nova (o `encryptDocument` filtra dígitos e destruiria o nome); IP em plaintext (legítimo interesse, retenção 5a); **`CONSENT_VERSION` v1.0→v1.1** + nova seção PJ em `/privacidade` e cláusula em `/termos`. Schema: enum `CnpjSituacao` + 11 colunas (migração `20260620120000_add_kyb_fields_to_user`, idempotente). **Verificação:** tsc+lint+99 testes unit verdes; CI do PR 100% verde (incl. E2E Playwright); deploy de staging + E2E Regressão Contínua verdes; registro/privacidade/termos/upgrade-pj checados ao vivo no staging. **🚨 Incidente resolvido no deploy:** após o merge, o cadastro de usuário (`/api/auth/register`) deu **500 no staging** porque o `prisma migrate deploy` do pipeline **não aplicou a migração KYB** — o staging tem **drift de histórico** (6 migrações antigas constam "not applied" no `_prisma_migrations` mas seu schema já existe via `db execute` out-of-band) e o `migrate deploy` não alcança a nova, embora o job Deploy Staging passe ✓; o código novo (Prisma client com colunas KYB) subiu sem as colunas no banco. **Fix:** apliquei a migração idempotente à mão (`prisma db execute --url $DIRECT_URL` de `.env.staging-migrate` — a conexão pooler trava, usar DIRECT) + `prisma migrate resolve --applied`; register voltou a 201 e o E2E Regressão ficou verde. **Lição:** toda migração nova exige aplicação manual no staging (db execute + resolve) enquanto o drift do histórico não for reconciliado. Levantou ainda a **5ª questão da pauta do D4 (PLD/Lei 9.613 — somos "sujeito obrigado"?)**. **Validação:** roteiro de teste para os testers gerado em PDF (`docs/roteiro-teste-cadastro-pj.{md,pdf}` — 2 formas: upgrade PF→PJ no `/perfil` e cadastro direto PJ no `/cadastro/completar`, + fila admin, com testes negativos e checklist) e **entregue aos testers — aguardando feedback**. **Pendências ao retomar:** tratar o feedback dos testers; commitar este `STATUS.md` (editado, não commitado p/ não re-disparar deploy durante a validação) + versionar o roteiro; reconciliar o drift do histórico de migrações do staging; limpar usuários de teste `kyb-fix-*@shareo-test.com`; H2 (circuit breaker, cron de retry da fila, job de retenção do IP). Produção segue gated por **D4**. Ver memórias [[project-kyb-leve-pj]] e [[project-d4-juridico]].
