...
/**
 * Borda neon animada — componente visual puro (Originkit), sem lógica de
 * produto. Renderiza um contorno luminoso que percorre o perímetro do
 * elemento pai; usar como camada absoluta atrás do conteúdo real:
 *
 *   <div className="relative">
 *     <div className="pointer-events-none absolute inset-0" aria-hidden>
 *       <NeonBorder color="#007B3C" />
 *     </div>
...
export default function NeonBorder(props: Props) {
  ...
  useEffect(() => {
    // Respeita a preferência do sistema — não existe outro lugar no projeto
    // que trate `prefers-reduced-motion` hoje (só em config de teste visual),
    // mas esta é uma animação contínua e decorativa; não tem por que ignorar
    // o sinal de quem pediu menos movimento. Não precisa desenhar nada aqui:
    // o `--arc` inicial de `glowGroup` (abaixo) já nasce correto a partir do
    // `size` do render, incluindo depois que o ResizeObserver o atualiza —
    // só precisa não ser sobrescrito pelo loop de animação.
    if (
  ...
  const radius =
    (Math.max(0, Math.min(100, rounded)) / 100) * (Math.min(size.w, size.h) / 2)
