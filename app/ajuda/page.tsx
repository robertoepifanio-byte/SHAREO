...
interface HelpVars {
  ...
  bemMaxLabel:   string // "R$ 1.000" — teto do valor do bem anunciado
...
function buildLocadorSteps(v: HelpVars): Step[] { return [
  ...
  },
  {
    ...
    example: `1ª locação: R$ 120/dia × 2 dias = ${formatPriceShort(24000)}. Taxa de plataforma (${v.feeLabel}) = ${formatPriceShort(calcSplit(24000, v.feeRateBps).platformFeeAmount)}. Você recebe ${formatPriceShort(calcSplit(24000, v.feeRateBps).ownerNetAmount)}, ${v.payoutLabel} após a confirmação da devolução. 2ª locação: mesmo fluxo — ao confirmar o recebimento a reserva é concluída, aquelas datas voltam a ficar livres e o anúncio segue no ar, sem precisar recadastrar nada.`,
  ...
  },
  {
    ...
    faqs: [
      ...
      { q: "O que acontece se meu item for extraviado?",
        a: `Em caso de furto ou extravio durante a locação, abra uma disputa na plataforma e registre um boletim de ocorrência (BO). A equipe ShareO analisa o caso, decide sobre a retenção ou o estorno do valor da locação e pode suspender a conta responsável. O ShareO não possui seguro contratado: não há cobertura do valor do bem além do que a mediação decidir sobre a própria locação. É por isso que, nesta fase, a plataforma limita o cadastro de itens a bens de até ${v.bemMaxLabel}.` },
  ...
  },
  {
    ...
    faqs: [
      ...
      { q: "Como reporto um usuário ou anúncio suspeito?",
        a: "Escreva para suporte@shareo.com.br com o link do anúncio ou do perfil e a descrição do problema. Anexe o que ajudar a entender o caso — capturas de tela do chat, fotos, o número da reserva. A equipe analisa e, quando é o caso, suspende o anúncio ou a conta. Se o problema for dentro de uma locação sua, use 'Reportar problema' na página da reserva." },
      { q: "Tenho um problema urgente com uma reserva em andamento. O que faço?",
        a: "Use o chat da própria reserva para falar com a outra parte — é o caminho mais rápido, e a maioria dos problemas se resolve ali. Se não resolver, ou em situações críticas (item não entregue no dia, sem contato com a outra parte, suspeita de golpe), escreva para suporte@shareo.com.br com o número da reserva: casos urgentes têm meta de resposta de até 4 horas úteis (segunda a sexta, 09h–17h). Havendo dano ou descumprimento, use 'Reportar problema' na página da reserva para abrir uma disputa." },
...
export default async function AjudaPage() {
  ...
  const v: HelpVars = {
    ...
    bemMaxLabel:   formatPriceShort(MAX_ITEM_VALUE_CENTS),
