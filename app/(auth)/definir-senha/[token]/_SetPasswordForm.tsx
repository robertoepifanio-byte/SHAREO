...
export function SetPasswordForm({ token }: { token: string }) {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      // Reaproveita o endpoint de redefinição (mesmo mecanismo de token).
      ...
      if (!res.ok) {
        setError(json.error?.message ?? "Erro ao definir senha.")
        return
      }
  ...
  if (success) {
    return (
      <div className="rounded-lg border border-border bg-surface p-8 shadow-card text-center">
        ...
        <h2 className="text-xl font-bold text-primary">Senha definida!</h2>
        <p className="mt-2 text-sm text-muted-foreground">Tudo pronto — redirecionando para o login…</p>
      </div>
    )
  }
